...
// ========================================================================================
// 🔹 THUMBNAIL SYSTEM - COMPLETE MODULE
// ========================================================================================
...
/**
 * Fallback function to get album art without thumbnail cache
 * @param {Object} metadb - foobar2000 metadb object
 * @param {number} size - desired size
 * @returns {Object|null} - GDI image object or null
 */
function getFallbackAlbumArt(metadb, size) {
    if (!metadb) return null;
    
    try {
        var albumArt = utils.GetAlbumArtV2(metadb, 0);
        if (albumArt && size) {
            return albumArt.Resize(size, size);
        }
        return albumArt;
    } catch (e) {
        console.log("❌ Error getting fallback album art:", e.message);
        return null;
    }
}
...
console.log("✅ Thumbnail System Module Ready");