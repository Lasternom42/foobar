...
// Font creation with scaling
function _gdiFont(name, size, style) {
    return gdi.Font(name, size, style || 0);
}
...
function _button(x, y, w, h, img_src, fn, tiptext) {
	...
	// Modified cs method to use selective repainting
	this.cs = (s) => {
		if (s == 'hover') {
		...
		// Use RepaintRect instead of window.Repaint for better performance
...
function _drawImage(gr, img, src_x, src_y, src_w, src_h, aspect, border, alpha) {
	if (!img || typeof img !== 'object') {
        console.log("Invalid image object passed to _drawImage.");
...
function _tt(value) {
	try {
		if (typeof tooltip !== 'undefined' && tooltip) {
			...
			// Fallback: create tooltip if it doesn't exist
			if (typeof window !== 'undefined' && window.CreateTooltip) {
				tooltip = window.CreateTooltip('Segoe UI', 12);
...
// Alternative volume position calculation that might work better
function calculateVolumePosition(volume, maxWidth) {
	...
	// Using a more user-friendly curve
	var linear = Math.pow(10, volume / 20);
	return Math.max(0, Math.min(maxWidth, Math.ceil(maxWidth * linear)));
}
